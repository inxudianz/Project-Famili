...
class OrderLandingViewController: UIViewController, OrderLandingViewProtocol {
    ...
    // MARK: - Initializer
    init() {
        super.init(nibName: String(describing: OrderLandingViewController.self), bundle: Bundle(for: OrderLandingViewController.self))
    }
    ...
    public func setupOrderView() {
        ...
        viewModel?.delegation = OrderLandingDelegate(orderView: orderView)
        ...
        orderView.delegate = viewModel?.delegation
